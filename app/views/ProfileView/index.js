...
class ProfileView extends React.Component {
  ...
  submit = async () => {
    ...
    const { name, username, email, newPassword, currentPassword, avatar, customFields } =
      this.state;
      ...
      onPress: async () => {
        try {
          ...
          EventEmitter.emit(LISTENER, {
            message: I18n.t("Logged_out_of_other_clients_successfully")
          });
        } catch {
  ...
  render() {
    const { name, username, email, newPassword, avatarUrl, customFields, avatar, saving } =
      this.state;
    const {
      user,
      theme,
