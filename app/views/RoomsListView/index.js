...
class RoomsListView extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    // eslint-disable-next-line react/destructuring-assignment
    const sortPreferencesUpdate = sortPreferencesShouldUpdate.some(
    ...
    this.querySubscription = observable.subscribe(data => {
      ...
      if (groupByType) {
        ...
        const channels = chats.filter(
          s => (s.t === "c" || s.t === "p") && !filterIsDiscussion(s) && !filterIsTeam(s)
        );
  ...
  renderSectionHeader = header => {
    ...
    return (
      <View
        style={[styles.groupTitleContainer, { backgroundColor: themes[theme].backgroundColor }]}>
